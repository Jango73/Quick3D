...
void CLogManager::initialize(QString sPath, QString sFileName, CXMLNode xParameters)
{
    ...
    if (xParameters.attributes()[ParamName_Level].isEmpty() == false)
    {
        setLevel(xParameters.attributes()[ParamName_Level]);
    }

    if (xTokensNode.attributes()[ParamName_Display].isEmpty() == false)
    {
        setDisplayTokens(xTokensNode.attributes()[ParamName_Display]);
    }

    if (xTokensNode.attributes()[ParamName_Ignore].isEmpty() == false)
    {
        setIgnoreTokens(xTokensNode.attributes()[ParamName_Ignore]);
    }

    if (xBackupNode.attributes()[ParamName_Active].isEmpty() == false)
    {
        m_bBackupActive = (bool) xBackupNode.attributes()[ParamName_Active].toInt();
    }
