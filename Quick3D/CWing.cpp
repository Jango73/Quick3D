...
void CWing::loadParameters(CXMLNode xComponent)
{
	...
	if (xPhysicsNode.m_vAttributes["AreaM2"].isEmpty() == false)
	{
		m_dArea_m2 = xPhysicsNode.m_vAttributes["AreaM2"].toDouble();
	}

	if (xPhysicsNode.m_vAttributes["Angle"].isEmpty() == false)
	{
		m_dWingAngle_rad = Math::Angles::toRad(xPhysicsNode.m_vAttributes["Angle"].toDouble());
	}

	if (xPhysicsNode.m_vAttributes["AileronAreaM2"].isEmpty() == false)
	{
		m_dAileronArea_m2 = xPhysicsNode.m_vAttributes["AileronAreaM2"].toDouble();
	}

	if (xPhysicsNode.m_vAttributes["AileronMaxPositiveAngle"].isEmpty() == false)
	{
		m_dAileronMaxPositiveAngle_rad = Math::Angles::toRad(xPhysicsNode.m_vAttributes["AileronMaxPositiveAngle"].toDouble());
	}

	if (xAileronPositionNode.isEmpty() == false)
	{
		m_vAileronPosition = CVector3(
			xAileronPositionNode.m_vAttributes[ParamName_x].toDouble(),
			xAileronPositionNode.m_vAttributes[ParamName_y].toDouble(),
			xAileronPositionNode.m_vAttributes[ParamName_z].toDouble()
		);
	}
}
