...
QVector<CComponent*> CComponentLoader::load(C3DScene* pScene, QString sFileName)
{
	...
	CXMLNode tDoc = CXMLNode::loadFromFile(sFileName);

	QString sCamera1;
	QString sCamera2;
	QString sControlled;

	foreach (CXMLNode xNode, tDoc.m_vNodes)
	{
		if (xNode.m_sTag == "Camera1")
		{
			sCamera1 = xNode.m_vAttributes[ParamName_Name];
		}
		else if (xNode.m_sTag == "Camera2")
		{
			sCamera2 = xNode.m_vAttributes[ParamName_Name];
		}
		else if (xNode.m_sTag == "ControlledComponent")
		{
			sControlled = xNode.m_vAttributes[ParamName_Name];
		}
		else if (xNode.m_sTag == ParamName_Components)
		{
...
{
	CXMLNode xComponent = CXMLNode::loadFromFile(sFileName);
...
{
	QString sClass = xComponent.m_vAttributes[ParamName_Class];
	...
	if (pComponent != NULL)
	{
		...
		{
			QString sControllerClass = xController.m_vAttributes[ParamName_Class];
